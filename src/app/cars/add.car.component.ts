...
export class AddCarComponent {
  ...
  constructor(private carsActions: CarsActions, private ngRedux: NgRedux<IAppState>, private router: Router) {

  }

  onSubmit() {
    this.carsActions.addCar(this.car);
    const subscription = this.ngRedux.select(state => state.cars)
      .subscribe(cars => {
        if (cars.carAdded) {
